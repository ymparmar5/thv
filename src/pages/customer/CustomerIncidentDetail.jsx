...
import {
  ...
  Video, Image as ImageIcon, FileText, Store, Camera, ShieldAlert,
  Play, ThumbsUp, X
} from 'lucide-react'
...
const statusConfig = {
  sent_to_customer: { label: 'Needs Your Review', color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400', dot: 'bg-amber-500' },
...
const CustomerIncidentDetail = () => {
  const { id } = useParams()
  const navigate = useNavigate()
  const { getIncidentById, getStoreById, getCameraById, getRegulationById, dispatch } = useIncidents()
  ...
  if (!incident) {
    return (
      <div className="text-center py-20 animate-fade-in">
        ...
        <Link to="/customer/incidents" className="text-primary-900 hover:underline text-sm font-bold">← Back to Incidents</Link>
      </div>
    )
  }
  ...
  const sCfg = statusConfig[incident.status] || statusConfig.sent_to_customer
  const svCfg = severityConfig[incident.severity] || severityConfig.medium

  const handleAcknowledge = () => {
    dispatch({
      type: 'ACKNOWLEDGE_INCIDENT',
      payload: { incidentId: incident.id, acknowledgedBy: currentCustomer.name + ' Admin' },
    })
  ...
  const handleRequestResolve = () => {
    dispatch({
      ...
      payload: {
        incidentId: incident.id,
        by: currentCustomer.name + ' Admin',
        message: 'We have reviewed and taken the necessary actions. Please consider this incident resolved.',
        newStatus: 'acknowledged',
      },
    })
    // The employee on the admin side would then mark it resolved
    dispatch({
      type: 'RESOLVE_INCIDENT',
      payload: { incidentId: incident.id, resolvedBy: currentCustomer.name + ' Admin', note: 'Client confirmed resolution' },
    })
  }

  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in pb-12">
      {/* Acknowledge Success Banner */}
      {showAckSuccess && (
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-900/30 rounded-2xl p-4 flex items-center space-x-3 animate-slide-up">
          <div className="w-10 h-10 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center flex-shrink-0">
            <CheckCircle className="w-5 h-5 text-green-600" />
          </div>
          <div>
            <p className="font-bold text-green-700 dark:text-green-400">Incident Acknowledged!</p>
            <p className="text-sm text-green-600 dark:text-green-400/80">You can now add feedback or request resolution.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <button onClick={() => navigate('/customer/incidents')} className="flex items-center text-sm text-gray-500 dark:text-gray-400 hover:text-primary-900 dark:hover:text-white font-medium mb-3 transition-colors">
        ...
        <div className="flex gap-2 flex-wrap">
          {incident.status === 'sent_to_customer' && (
            <button onClick={handleAcknowledge} className="btn-primary text-sm py-2 px-4">
              <ThumbsUp className="w-4 h-4 mr-1.5" /> Acknowledge Incident
            </button>
          )}
          {incident.status === 'acknowledged' && (
            <button onClick={handleRequestResolve} className="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-xl text-sm font-bold hover:bg-green-700 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card p-6">
            ...
            {regulation && (
              <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-200 dark:border-red-900/30 flex items-start space-x-2">
                ...
                <div>
                  <p className="text-xs text-red-500 font-bold">Regulation Violated</p>
          ...
          <div className="card p-6">
            ...
            {incident.reason && (
              <div className="mt-4 pt-4 border-t border-gray-100 dark:border-secondary-700">
                <p className="text-xs text-gray-500 dark:text-gray-400 font-bold mb-1">Root Cause</p>
          ...
          <div className="card p-6">
            ...
            {incident.customerFeedback?.length > 0 && (
              <div className="space-y-3 mb-4">
                {incident.customerFeedback.map((fb, i) => (
                  <div key={i} className="bg-primary-50/50 dark:bg-primary-900/10 rounded-xl p-4 border border-primary-100 dark:border-primary-900/30">
            ...
            {incident.status !== 'resolved' && (
              <div className="space-y-3">
                <textarea
                  ...
                  placeholder={
                    incident.status === 'sent_to_customer'
                      ? 'Please acknowledge this incident first before adding feedback...'
                      : 'Add your feedback, action taken, or request resolution...'
                  }
                  disabled={incident.status === 'sent_to_customer'}
                  className="w-full px-4 py-3 bg-gray-50 dark:bg-secondary-900 border border-gray-200 dark:border-secondary-700 rounded-xl text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-900/20 focus:border-primary-900 transition-all resize-y disabled:opacity-50 disabled:cursor-not-allowed"
                />
                <div className="flex justify-end">
                  <button
                    onClick={handleAddFeedback}
                    disabled={!feedbackText.trim() || incident.status === 'sent_to_customer'}
                    className="btn-primary text-sm py-2 px-6 disabled:opacity-40 disabled:cursor-not-allowed disabled:transform-none"
                  >
            ...
            {incident.status === 'resolved' && (
              <div className="text-center py-4 bg-green-50 dark:bg-green-900/10 rounded-xl border border-green-200 dark:border-green-900/30">
                <CheckCircle className="w-8 h-8 text-green-500 mx-auto mb-2" />
                <p className="text-sm font-bold text-green-700 dark:text-green-400">This incident has been resolved</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="card p-6">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${svCfg.color}`}>
                  {svCfg.label}
                </span>
              </div>
            </div>
          ...
          {incident.status !== 'resolved' && (
            <div className="card p-6">
              ...
              <div className="space-y-2">
                {incident.status === 'sent_to_customer' && (
                  <button onClick={handleAcknowledge} className="w-full flex items-center justify-center px-4 py-3 bg-primary-900 text-white rounded-xl text-sm font-bold hover:bg-primary-700 transition-colors">
                    <ThumbsUp className="w-4 h-4 mr-2" /> Acknowledge
                  </button>
                )}
                {incident.status === 'acknowledged' && (
                  <>
                    <button
                      ...
                      className="w-full flex items-center justify-center px-4 py-3 border border-primary-900 text-primary-900 dark:text-primary-400 rounded-xl text-sm font-bold hover:bg-primary-50 dark:hover:bg-primary-900/10 transition-colors"
                    >
                      <MessageSquare className="w-4 h-4 mr-2" /> Add Feedback
                    </button>
                    <button onClick={handleRequestResolve} className="w-full flex items-center justify-center px-4 py-3 bg-green-600 text-white rounded-xl text-sm font-bold hover:bg-green-700 transition-colors">
                      <CheckCircle className="w-4 h-4 mr-2" /> Close & Resolve
                    </button>
                  </>
                )}
              </div>
...
export default CustomerIncidentDetail
